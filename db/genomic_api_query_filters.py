...
# Modelled after vdjbase_api_query_filters.py

from sqlalchemy import func
from db.genomic_airr_model import Sample, Subject, Study, TissuePro, SeqProtocol, DataPro
from db.genomic_model import Sequence, Gene
...
sample_info_filters = {    
    ...
    'subject_id': {'model': 'Subject', 'field': Subject.subject_id, 'help': 'Subject ID assigned by submitter, unique within study', 'example': 'SUB856413' },
    'synthetic': {'model': 'Subject', 'field': Subject.synthetic, 'help': 'TRUE for libraries in which the diversity has been synthetically generated (e.g. phage display)', 'example': '' },
    'species_id': {'model': 'Subject', 'field': Subject.species_id, 'help': 'CURIE of the concept, encoding the ontology and the local ID', 'example': '' },
    'species_label': {'model': 'Subject', 'field': Subject.species_label, 'help': 'Label of the concept in the respective ontology', 'example': '' },
    'organism_id': {'model': 'Subject', 'field': Subject.organism_id, 'help': 'CURIE of the concept, encoding the ontology and the local ID', 'example': '' },
    'organism_label': {'model': 'Subject', 'field': Subject.organism_label, 'help': 'Label of the concept in the respective ontology', 'example': '' },
    'sex': {'model': 'Subject', 'field': Subject.sex, 'help': 'Biological sex of subject', 'example': 'female' },
    'age_min': {'model': 'Subject', 'field': Subject.age_min, 'help': 'Specific age or lower boundary of age range.', 'example': '60' },
    'age_max': {'model': 'Subject', 'field': Subject.age_max, 'help': 'Upper boundary of age range or equal to age_min for specific age. This field should only be null if age_min is null.', 'example': '80' },
    'age_unit_id': {'model': 'Subject', 'field': Subject.age_unit_id, 'help': 'CURIE of the concept, encoding the ontology and the local ID', 'example': '' },
    'age_unit_label': {'model': 'Subject', 'field': Subject.age_unit_label, 'help': 'Label of the concept in the respective ontology', 'example': '' },
    'age_event': {'model': 'Subject', 'field': Subject.age_event, 'help': 'Event in the study schedule to which `Age` refers. For NCBI BioSample this MUST be `sampling`. For other implementations submitters need to be aware that there is currently no mechanism to encode to potential delta between `Age event` and `Sample collection time`, hence the chosen events should be in temporal proximity.', 'example': 'enrollment' },
    'age': {'model': 'Subject', 'field': Subject.age, 'help': '', 'example': '' },
    'ancestry_population': {'model': 'Subject', 'field': Subject.ancestry_population, 'help': 'Broad geographic origin of ancestry (continent)', 'example': 'list of continents, mixed or unknown' },
    'ethnicity': {'model': 'Subject', 'field': Subject.ethnicity, 'help': 'Ethnic group of subject (defined as cultural/language-based membership)', 'example': 'English, Kurds, Manchu, Yakuts (and other fields from Wikipedia)' },
    'race': {'model': 'Subject', 'field': Subject.race, 'help': 'Racial group of subject (as defined by NIH)', 'example': 'White, American Indian or Alaska Native, Black, Asian, Native Hawaiian or Other Pacific Islander, Other' },
    'strain_name': {'model': 'Subject', 'field': Subject.strain_name, 'help': 'Non-human designation of the strain or breed of animal used', 'example': 'C57BL/6J' },
    'linked_subjects': {'model': 'Subject', 'field': Subject.linked_subjects, 'help': 'Subject ID to which `Relation type` refers', 'example': 'SUB1355648' },
    'link_type': {'model': 'Subject', 'field': Subject.link_type, 'help': 'Relation between subject and `linked_subjects`, can be genetic or environmental (e.g.exposure)', 'example': 'father, daughter, household' },
    'study_group_description': {'model': 'Subject', 'field': Subject.study_group_description, 'help': 'Designation of study arm to which the subject is assigned to', 'example': 'control' },
    'disease_diagnosis_id': {'model': 'Subject', 'field': Subject.disease_diagnosis_id, 'help': 'CURIE of the concept, encoding the ontology and the local ID', 'example': '' },
    'disease_diagnosis_label': {'model': 'Subject', 'field': Subject.disease_diagnosis_label, 'help': 'Label of the concept in the respective ontology', 'example': '' },
    'disease_length': {'model': 'Subject', 'field': Subject.disease_length, 'help': 'Time duration between initial diagnosis and current intervention', 'example': '23 months' },
    'disease_stage': {'model': 'Subject', 'field': Subject.disease_stage, 'help': 'Stage of disease at current intervention', 'example': 'Stage II' },
    'prior_therapies': {'model': 'Subject', 'field': Subject.prior_therapies, 'help': 'List of all relevant previous therapies applied to subject for treatment of `Diagnosis`', 'example': 'melphalan/prednisone' },
    'immunogen': {'model': 'Subject', 'field': Subject.immunogen, 'help': 'Antigen, vaccine or drug applied to subject at this intervention', 'example': 'bortezomib' },
    'intervention': {'model': 'Subject', 'field': Subject.intervention, 'help': 'Description of intervention', 'example': 'systemic chemotherapy, 6 cycles, 1.25 mg/m2' },
    'medical_history': {'model': 'Subject', 'field': Subject.medical_history, 'help': 'Medical history of subject that is relevant to assess the course of disease and/or treatment', 'example': 'MGUS, first diagnosed 5 years prior' },
    'receptor_genotype_set_id': {'model': 'Subject', 'field': Subject.receptor_genotype_set_id, 'help': 'A unique identifier for this Receptor Genotype Set.', 'example': '' },
    'mhc_genotype_set_id': {'model': 'Subject', 'field': Subject.mhc_genotype_set_id, 'help': 'A unique identifier for this MHC Genotype Set.', 'example': '' },
    'mhc_genotype_id': {'model': 'Subject', 'field': Subject.mhc_genotype_id, 'help': 'A unique identifier for this MHC Genotype, assumed to be unique in the context of the study.', 'example': '' },
    'genotype_class': {'model': 'Subject', 'field': Subject.genotype_class, 'help': '', 'example': 'MHC' },
    'gene_symbol': {'model': 'Subject', 'field': Subject.gene_symbol, 'help': 'The accepted name for this gene', 'example': '' },
    'germline_set_ref': {'model': 'Subject', 'field': Subject.germline_set_ref, 'help': 'Repository and list from which it was taken (issuer/name/version)', 'example': '' },
    'genotype_process': {'model': 'Subject', 'field': Subject.genotype_process, 'help': 'Information on how the genotype was acquired. Controlled vocabulary.', 'example': 'repertoire_sequencing' },
    'patient_name': {'model': 'Subject', 'field': Subject.patient_name, 'help': 'Subject name as allocated by VDJbase', 'example': '' },
