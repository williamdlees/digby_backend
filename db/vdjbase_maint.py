...
from db.vdjbase_model import Base, Details
from db.vdjbase_reference import import_reference_alleles, add_alias_sets
...
def create_single_database(job, species, dataset, upload_path, in_situ=False):
    ...
    try:
        ...
        job.update_state(state='PENDING', meta={'value': 'Processing alias sets'})
        result.extend(add_alias_sets(os.path.join(ds_dir, 'reference'), session))
